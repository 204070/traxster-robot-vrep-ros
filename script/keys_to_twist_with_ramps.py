...
def fetch_params(name, default):
    if rospy.has_param(name):
        ...
        rospy.logwarn("paremeter [%s] not defined; defaulting to %.3f" %(name, default))
        return default

if __name__ == '__main__':
    rospy.init_node('keys_to_twist', anonymous=True)
    ...
    g_vel_scales[0] = fetch_params('~angular_scale', 0.1)
    g_vel_scales[1] = fetch_params('~linear_scale', 0.1)
    g_vel_ramps[0] = fetch_params('~angular_accel', 1.0)
    g_vel_ramps[1] = fetch_params('~linear_accel', 1.0)
